...
body {
  background-image: url('https://github.com/twitch-bridge/twitch-bridge.github.io/blob/master/wyleia_logo10.png?raw=true');
...
summaryindiv <- function(output){
  ...
  indiv <- indiv[order(rowSums(indiv[,2:3])/indiv[,4], decreasing = TRUE),]
...
summaryteam <- function(indiv) {
  ...
  team <- data.frame(Team = c("Blue", "Green", "Orange", "Yellow", "Red", "Purple"),
                     ...
                     'Points/Match' = c(bluepts/bluematches, greenpts/greenmatches,
                                        orangepts/orangematches, yellowpts/yellowmatches,
                                        redpts/redmatches, purplepts/purplematches),
                     check.names = F)
  
  team$`Points/Match` <- format(round(team$`Points/Match`, 2), nsmall = 2)
  team$`Points/Match` <- as.numeric(as.character(team$`Points/Match`))
  team <- team[order(team$`Points/Match`, decreasing = TRUE),]
...
team.table %>%
  kable("html", row.names = FALSE, align = c("l","c","c","c")) %>%
  kable_styling(font_size = 18, full_width = F,
                bootstrap_options = c("striped", "hover")) %>%
...
## Sad Sortable Table (Individual)

```{r dt, echo = FALSE, warning = FALSE, message = FALSE}
datatable(indiv.table)
```

## Sad Sortable Table (Team)

```{r dt.team, echo = FALSE, warning = FALSE, message = FALSE}
