...
tables %>% 
  ...
  row_spec(which(tables$Player %in% blue), color = "#3498db") %>%
  row_spec(which(tables$Player %in% green), color = "#1f8b4c") %>%
  row_spec(which(tables$Player %in% yellow), color = "#f1c40f") %>%
  row_spec(which(tables$Player %in% purple), color = "#71368a") %>%
  row_spec(which(tables$Player %in% red), color = "red") %>%
  row_spec(which(tables$Player %in% orange), color = "orange") 
```
